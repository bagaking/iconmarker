...
> **状态说明（历史规划，不是当前契约）**
>
> 本文是项目早期增强规划和路线图记录，用于保留设计背景与候选能力。它不代表当前发布版本、README、Go API 或兼容性承诺；当前使用方式与 API 契约以 `README.md`、示例和已发布代码为准。下文的“目标”“技术要求”“关键接口设计”“评估指标”等内容应按 roadmap/历史规划解读，除非已在当前代码和 README 中实现并说明。

## 1. 项目概述与目标

Icon Marker 是一个用于在图像上添加文本的 Go 语言库，我们的增强目标是将其打造成一个专注于群组图标生成的高效工具。

> **实现状态（当前版本）**：本文同时记录已落地能力和未来路线。已落地的
> API 包括 JPEG 背景文字创建、SVG 渲染、内嵌资源、基础滤镜、LRU/TTL
> 缓存及批量 SVG 渲染；服务端、工作池、模板化群组图标和动画 SVG 仍属于
> 未来路线，不应被视为当前 API 承诺。
...
当前代码已提供并发安全的缓存、组件接口和 `go test`/`go test -race`/`go vet`
门禁；工作池、动态并行度和性能监控仍待后续迭代实现。

### 2.2 P0: SVG 支持（基础能力已落地）
...
`IconService` 尚未实现，以下方法是未来路线草案，不属于当前公开 API：
